...
class GETQuery:
    ...
    def _check_path(self, path):
        ...
        if not path == self.VALID_PATH:
            return f"Error: invalid path '{path}'. Expected '{self.VALID_PATH}'"
    ...
    def _check_params(self, params):
        ...
        for param in params:
            if not (param in self.VALID_QUERY_PARAMS):
                return f"Error: invalid query parameter '{param}'. Allowed parameters are: {', '.join(self.VALID_QUERY_PARAMS)}"
    ...
    def _check_attrs(self, attrs, valid_attrs):
        ...
        for attr in attrs:
            if not (attr in valid_attrs):
                return f"Error: invalid attribute '{attr}'. Allowed attributes are: {', '.join(valid_attrs)}"
    ...
    def _check_order(self, query_params):
        ...
        if not query_params.get('attribute'):
            ...
            if len(query_params['order']) > 1:
                return f"Error: {len(query_params['order'])} 'order' parameters given. 1 expected"
            else:
                # 'order' must have only 'asc' or 'desc' value
                order = query_params['order'][0]
                if not (order in self.VALID_ORDER_VALUES):
                    return f"Error: invalid order '{order}'. Allowed values are: {', '.join(self.VALID_ORDER_VALUES)}."
    ...
    def _check_offset(self, offset_list, cats_number):
        ...
        if len(offset_list) > 1:
            return f"Error: {len(offset_list)} 'offset' parameters given. 1 expected"
        else:
            offset = offset_list[0]
            # 'offset' must be integer
            if not offset.isdigit():
                return f"Error: invalid offset '{offset}'. Integer expected."
            else:
                # 'offset' must be < records in table otherwise nothing to output
                if int(offset) >= cats_number:
                    return f"Warning: There is no results because offset {offset} " \
                           f"greater than the maximum of {cats_number-1}"

    def _check_limit(self, limit_list):
        """ Check 'limit' value and add error message for every invalid case """
        # 'limit' must be only one
        if len(limit_list) > 1:
            return f"Error: {len(limit_list)} 'limit' parameters given. 1 expected"
        else:
            limit = limit_list[0]
            # 'limit' must be integer
            if not limit.isdigit():
                return f"Error: invalid limit '{limit}'. Integer expected."
